...
const WRONG_KEY='ccht_wrong_v4', STATS_KEY='ccht_stats_v4', HIST_KEY='ccht_hist_v4', PERF_KEY='ccht_perf_v4';
...
function pickQuestions(mode){
  ...
  // Also shuffle choices within each question for variety
  pool.forEach(q=>{
  ...
  if(mode==='booster'){
    // find weakest two section/topics from perf
    const tuples=[];
    ...
    const targets=tuples.slice(0,3); // up to 3 weakest
...
function showCurrent(){
  ...
  const choicesHtml = (q._choices||q.choices).map((c,i)=>`<div class="choice" data-i="${i}"><div class="badge">${String.fromCharCode(65+i)}</div><div>${c}</div></div>`).join('');
  card.innerHTML=`
  ...
  status.textContent=session.showImmediate?'Pick the best answer — explanation appears right away.':'Practice exam mode — answers at the end.';
